...
export function UpgradeMessage({ pageSlug: slug, trialEndDate, subscriptionEndedDate }: Props) {
	...
	if (subscriptionEndedDate) {
		const dateFormatted = dayjs(subscriptionEndedDate).locale('pt-br').format('DD[ de ]MMMM[ de ]YYYY')
		...
		return (
			<div className="border-sticky-border bg-sticky-background/85 sm:bg-sticky-background sticky top-0 right-0 left-0 z-10 gap-1 border-b px-6 py-2 text-center text-sm text-balance shadow-xs backdrop-blur-xs select-none sm:py-3 sm:text-base">
				<span>
					Sua página ficará visível para todos até dia <span className="font-bold">{dateFormatted}</span>.
				</span>{' '}
				<button
		...
		<div className="border-sticky-border bg-sticky-background/85 sm:bg-sticky-background sticky top-0 right-0 left-0 z-10 gap-1 border-b px-6 py-2 text-center text-sm text-balance shadow-xs backdrop-blur-xs select-none sm:py-3 sm:text-base">
			...
			<Link
				...
				className={cn(
					...
					trialExpired ? 'text-red-500 hover:text-red-600' : 'text-accent-green hover:text-accent-green-hover',
				)}
			>
